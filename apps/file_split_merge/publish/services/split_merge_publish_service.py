"""Publicar versión File Split/Merge (perfil + operation + sm_rules)."""
...
def _collect_enabled_rule_issues(source: dict) -> list[dict]:
    ...
    payload = (source.get("config") or {}).get(rules_svc.CONFIG_KEY) or {}
    ...
    for item in raw_rules:
        ...
        errors = validation.validate_rule(
            rule, field_names=field_names, operation=operation
        )
        ...
        issues.append(
            {
                "id": rule.get("id"),
                "code": rule.get("code"),
                "field_name": (rule.get("params") or {}).get("field_name") or "",
                "messages": messages,
...
def validate_for_publish(project: Project) -> OperationResult | None:
    ...
    if not rules_svc.is_rules_complete(project):
        return OperationResult.failure(
            "validation_form",
            MSG_NO_RULES,
...
def get_checklist(project: Project, membership=None) -> list[ChecklistItem]:
    ...
    rules_ready = bool(rules_ctx.get("rules_complete")) and not issues

    operation = rules_ctx.get("operation") or ""
    op_label = catalog.OPERATION_LABELS.get(operation, "Sin operación")
    ...
    return [
        ...
        ChecklistItem(
            ...
            rules_ready,
            (
                f"{op_label} · {rules_ctx['rules_enabled_count']} ON · "
                f"{rules_ctx['rules_count']} total"
                if rules_ready
                else (
                    f"{len(issues)} regla(s) inválida(s)"
                    if issues
                    else "Complete operación Split/Merge y reglas mínimas"
                )
            ),
            "file_split_merge:rules_hub",
        ),
    ]
...
def get_hub_context(user, project: Project, membership=None) -> PublishHubContext:
    ...
    summary = {
        ...
        "operation": operation,
        "operation_label": catalog.OPERATION_LABELS.get(operation, "Sin operación"),
...
@transaction.atomic
def publish_sm_definition(user, project: Project) -> OperationResult:
    ...
    try:
        ...
        logger.exception("publish_sm_definition unexpected project=%s", project.slug)
