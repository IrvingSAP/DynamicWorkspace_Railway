...
FORM_CREATE = "file_split_merge:project:create"

MSG_NO_ACCESS = split_merge_project_service.MSG_NO_ACCESS
MSG_PA_ONLY = split_merge_project_service.MSG_PA_ONLY


def _form_namespace(action: str, project_slug=None) -> str:
    if project_slug:
        return f"file_split_merge:project:{action}:{project_slug}"
    return f"file_split_merge:project:{action}"
...
def project_list(request):
    rows, stats = split_merge_project_service.list_with_stats(request.user)
    company = request.user.profile.company
    ctx = _sidebar_context()
    ctx.update({"rows": rows, "stats": stats, "company": company})
    return render(request, "file_split_merge/projects/list.html", ctx)


@_sm_view
def project_list_help(request):
    ctx = _sidebar_context()
    return render(request, "file_split_merge/projects/list_help.html", ctx)


@_sm_view
def project_create_help(request):
    ctx = _sidebar_context()
    return render(request, "file_split_merge/projects/create_help.html", ctx)
...
def project_create(request):
    company = request.user.profile.company
    posted = split_merge_project_service.default_posted()
    ...
    if request.method == "POST":
        posted = split_merge_project_service.posted_from_request(request.POST)
        result = split_merge_project_service.create_project(request.user, posted)
        if result.ok:
            clear_form_state(request, FORM_CREATE)
            messages.success(request, result.user_message)
            project = result.payload["project"]
            return redirect("file_split_merge:project_hub", project_slug=project.slug)
        errors = result.errors or {}
        stash_form_state(request, FORM_CREATE, posted, errors)
        messages.error(request, result.user_message)
        return redirect("file_split_merge:project_create")

    ctx = _sidebar_context()
    ctx.update({"posted": posted, "errors": errors, "company": company})
    return render(request, "file_split_merge/projects/create.html", ctx)
...
def project_hub(request, project_slug: str):
    ...
    hub = split_merge_project_service.get_hub_context(request.user, project)
    ...
    return render(request, "file_split_merge/projects/hub.html", ctx)
...
def project_hub_help(request, project_slug: str):
    ...
    return render(request, "file_split_merge/projects/hub_help.html", ctx)
...
def project_members(request, project_slug: str):
    ...
    if not project_service.user_can_manage_members(request.user, project):
        ...
        return redirect("file_split_merge:project_hub", project_slug=project_slug)
    ...
    if request.method == "POST":
        ...
        if action == "invite":
            ...
            if result.ok:
                ...
                messages.error(request, result.user_message)
            return redirect("file_split_merge:project_members", project_slug=project_slug)
    ...
    return render(request, "file_split_merge/projects/members.html", ctx)
...
def project_members_help(request, project_slug: str):
    ...
    ctx = _sidebar_context()
    ctx.update({"project": project, "company": project.company})
    return render(request, "file_split_merge/projects/members_help.html", ctx)
