"""Historial File Split/Merge — Módulo 6 (sm_history.md).

Lista y detalla SplitMergeJob. TTL vía sm_run_service.
...
from apps.file_split_merge.run.services import sm_run_service as run_svc
...
STATUS_ALL = "all"
STATUS_FILTERABLE = {
    SplitMergeJob.STATUS_COMPLETED,
    SplitMergeJob.STATUS_FAILED,
}

OPERATION_ALL = "all"

NON_FINAL_STATUSES = (SplitMergeJob.STATUS_QUEUED, SplitMergeJob.STATUS_RUNNING)
...
def user_can_view_history(user, project: Project) -> bool:
    ...
    return split_merge_project_service.user_can_view(user, project)
...
def status_options() -> list[dict]:
    return [
        {
            "value": SplitMergeJob.STATUS_COMPLETED,
            "label": run_svc.STATUS_LABELS[SplitMergeJob.STATUS_COMPLETED],
        },
        {
            "value": SplitMergeJob.STATUS_FAILED,
            "label": run_svc.STATUS_LABELS[SplitMergeJob.STATUS_FAILED],
        },
    ]


def operation_options() -> list[dict]:
    return [
        {"value": catalog.OPERATION_SPLIT, "label": catalog.OPERATION_LABELS[catalog.OPERATION_SPLIT]},
        {"value": catalog.OPERATION_MERGE, "label": catalog.OPERATION_LABELS[catalog.OPERATION_MERGE]},
    ]
...
def parse_filters(params) -> tuple[dict, dict]:
    ...
    operation = (params.get("operation") or OPERATION_ALL).strip()
...
def has_active_filters(filters: dict) -> bool:
    return bool(
        ...
        or filters["operation"] != OPERATION_ALL
...
def _querystring(filters: dict, **overrides) -> str:
    data = {
        ...
        "operation": filters["operation"] if filters["operation"] != OPERATION_ALL else "",
...
def _filtered_queryset(project: Project, filters: dict):
    ...
    if filters["operation"] != OPERATION_ALL:
    ...
    # filename / hash live in JSON inputs/outputs — filter in Python after scan
    return qs


def _matches_filename_hash(job: SplitMergeJob, filename: str, hash_q: str) -> bool:
    if not filename and not hash_q:
        return True
    inputs = job.inputs or []
    outputs = job.outputs or []
    if filename:
        needle = filename.lower()
        names = [str(i.get("filename") or "").lower() for i in inputs if isinstance(i, dict)]
        names += [str(o.get("filename") or "").lower() for o in outputs if isinstance(o, dict)]
        if not any(needle in n for n in names):
            return False
    if hash_q:
        needle = hash_q.lower()
        hashes = [str(i.get("content_hash") or "").lower() for i in inputs if isinstance(i, dict)]
        hashes += [str(o.get("content_hash") or "").lower() for o in outputs if isinstance(o, dict)]
        if not any(h.startswith(needle) for h in hashes if h):
            return False
    return True


def can_delete_job(user, job: SplitMergeJob) -> bool:
...
def _job_storage_root(project: Project, job: SplitMergeJob):
    return storage_service.job_input_dir(
        project.company_id, project.id, job.id
    ).parent


def _purge_job_storage(project: Project, job: SplitMergeJob) -> None:
...
def _own_jobs_queryset(user, project: Project):
    return (
        SplitMergeJob.objects.filter(project=project, executed_by=user)
        .exclude(status__in=NON_FINAL_STATUSES)
        .order_by("-created_at")
    )
...
def delete_own_job(user, project: Project, job_id) -> dict:
    ...
    job = (
        SplitMergeJob.objects.filter(project=project, pk=job_id)
        .exclude(status__in=NON_FINAL_STATUSES)
        .first()
    )
    if job is None:
...
def _primary_input(job: SplitMergeJob) -> dict:
    inputs = job.inputs or []
...
def build_row(project: Project, job: SplitMergeJob, *, user=None) -> dict:
    metrics = job.metrics or {}
    expired = run_svc.is_download_expired(job)
    primary = _primary_input(job)
    content_hash = primary.get("content_hash") or ""
    ...
    has_outputs = bool(job.outputs) and not job.dry_run
    can_dl = (
        user is not None
    ...
    return {
        ...
        "filename": filename,
        "size_label": detection_service.human_size(int(primary.get("size_bytes") or 0)),
        ...
        "detail_url": reverse(
            "file_split_merge:history_detail",
            kwargs={"project_slug": project.slug, "job_id": job.id},
        ),
    }
...
def executed_by_options(project: Project) -> list[dict]:
    users = (
        get_user_model()
        .objects.filter(file_split_merge_jobs__project=project)
        .exclude(file_split_merge_jobs__status__in=NON_FINAL_STATUSES)
        .distinct()
        .order_by("email")
    )
    return [
...
def build_history_context(user, project: Project, params) -> dict:
    ...
    if filters["filename"] or filters["hash"]:
        jobs = [
            j
            for j in jobs
            if _matches_filename_hash(j, filters["filename"], filters["hash"])
        ]
...
def build_detail_context(user, project: Project, job: SplitMergeJob) -> dict:
    """Proyección estable de detalle (API-ready)."""
    row = build_row(project, job, user=user)
    view = run_svc.build_job_view(project, job)
    role = resolve_role(user, project)
    can_dl = (
        run_svc.user_can_download(user, project)
        and not row["is_expired"]
        and not job.dry_run
    )

    snapshot = job.rules_snapshot or {}
    ...
    return {
        ...
        "rules_enabled": [
            {
                "code": r.get("code") or "",
                "params_summary": catalog.params_summary(
                    r.get("code") or "", r.get("params")
                ),
