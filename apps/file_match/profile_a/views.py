...
STEP4_TEMPLATES = {
    "fixed": "file_match/profile_a/step4_fields.html",
    "delimited": "file_match/profile_a/step4_fields_delimited.html",
    "xlsx": "file_match/profile_a/step4_fields_xlsx.html",
    "json": "file_match/profile_a/step4_fields_json.html",
...
def _base_context(request, project, current_step: int | None = None) -> dict:
    ...
    wizard = profile_a_wizard_service.get_wizard_context(project, membership)
    source = source_persistence_service.get_source_dict(project)
    seed_ctx = profile_seed_service.get_profile_a_seed_context(request.user, project)
    return {
        "project": project,
        "wizard": wizard,
        ...
        "source_save_url": reverse(
            "file_match:profile_a_save", kwargs={"project_slug": project.slug}
        ),
        **seed_ctx,
    }
...
def hub(request, project_slug: str):
    return _render(request, project_slug, "file_match/profile_a/hub.html")


@_profile_a_view
def hub_help(request, project_slug: str):
    return _render(request, project_slug, "file_match/profile_a/hub_help.html")


@_profile_a_view
def profile_a_seed_hub(request, project_slug: str):
    project = _get_project_or_redirect(request, project_slug)
    if project is None:
        return redirect("file_match:project_list")
...
@_profile_a_view
def profile_a_seed_hub_help(request, project_slug: str):
    project = _get_project_or_redirect(request, project_slug)
    if project is None:
        return redirect("file_match:project_list")
...
@_profile_a_view
def profile_a_seed_picker(request, project_slug: str):
    project = _get_project_or_redirect(request, project_slug)
    if project is None:
        return redirect("file_match:project_list")
    if not profile_seed_service.user_can_import(request.user, project):
...
@_profile_a_view
def profile_a_seed_picker_help(request, project_slug: str):
    project = _get_project_or_redirect(request, project_slug)
    if project is None:
        return redirect("file_match:project_list")
...
def profile_a_seed_apply(request, project_slug: str):
    project = _get_project_or_redirect(request, project_slug)
    if project is None:
        return redirect("file_match:project_list")
    if not profile_seed_service.user_can_import(request.user, project):
    ...
    if request.method == "POST":
        ...
        if action != "apply":
            ...
            return redirect(
                "file_match:profile_a_seed_picker", project_slug=project_slug
            )
        result = apply_seed_service.apply_seed_to_profile_a(
...
@_profile_a_view
def profile_a_seed_apply_help(request, project_slug: str):
    project = _get_project_or_redirect(request, project_slug)
    if project is None:
        return redirect("file_match:project_list")
...
@_profile_a_view
def profile_a_seed_history(request, project_slug: str):
    project = _get_project_or_redirect(request, project_slug)
    if project is None:
        return redirect("file_match:project_list")
...
@_profile_a_view
def profile_a_seed_history_detail(request, project_slug: str, event_id):
    project = _get_project_or_redirect(request, project_slug)
    if project is None:
        return redirect("file_match:project_list")
    ...
    if detail is None:
        ...
        return redirect(
            "file_match:profile_a_seed_history", project_slug=project_slug
        )
    return _render(
        request,
        project_slug,
...
@_profile_a_view
def profile_a_seed_history_help(request, project_slug: str):
    project = _get_project_or_redirect(request, project_slug)
    if project is None:
        return redirect("file_match:project_list")
...
def step1_help(request, project_slug: str):
    return _render(request, project_slug, "file_match/profile_a/step1_help.html", current_step=1)


@_profile_a_view
def step2_help(request, project_slug: str):
    return _render(request, project_slug, "file_match/profile_a/step2_help.html", current_step=2)


@_profile_a_view
def step3_help(request, project_slug: str):
    return _render(request, project_slug, "file_match/profile_a/step3_help.html", current_step=3)
...
def step5_help(request, project_slug: str):
    return _render(request, project_slug, "file_match/profile_a/step5_help.html", current_step=5)


@_profile_a_view
def step6_help(request, project_slug: str):
    return _render(request, project_slug, "file_match/profile_a/step6_help.html", current_step=6)
...
def step1_file_type(request, project_slug: str):
    ...
    ctx.update(profile_a_wizard_service.get_step1_catalog_context())
    return render(request, "file_match/profile_a/step1_file_type.html", ctx)
...
def step2_capture_start(request, project_slug: str):
    ...
    return render(request, "file_match/profile_a/step2_capture_start.html", ctx)
...
def step3_capture_end(request, project_slug: str):
    ...
    return render(request, "file_match/profile_a/step3_capture_end.html", ctx)
...
def step5_content_rules(request, project_slug: str):
    ...
    return render(request, "file_match/profile_a/step5_content_rules.html", ctx)
...
def step6_report(request, project_slug: str):
    ...
    return render(request, "file_match/profile_a/step6_report.html", ctx)


@_profile_a_view
@require_http_methods(["POST"])
def profile_a_save(request, project_slug: str):
    project = _get_project_or_redirect(request, project_slug)
    if project is None:
    ...
    if raw:
        try:
            ...
            if request.headers.get("X-Requested-With") == "XMLHttpRequest":
                return JsonResponse(
                    {"ok": False, "message": "JSON de perfil A inválido."},
                    status=400,
                )
            messages.error(request, "JSON de perfil A inválido.")
            return redirect("file_match:profile_a_hub", project_slug=project_slug)
    else:
        for key in (
            "file_type_code",
    ...
    if not redirect_to:
        redirect_to = reverse("file_match:profile_a_hub", kwargs={"project_slug": project_slug})
