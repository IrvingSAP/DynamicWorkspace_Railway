"""Contexto del asistente de perfil A (profile_a.md)."""

from dataclasses import dataclass, field

from apps.dms.source_profile.services import source_profile_catalog_service, source_profile_service
from apps.file_match.profile_a.services.profile_a_whitelist import PROFILE_A_FILE_TYPE_WHITELIST
...
class ProfileAWizardContext:
    ...
    continue_step_url_name: str = "file_match:profile_a_step1"


_STEP_META = (
    (1, "paso-1", "Paso 1 — Tipo de archivo A", "file_match:profile_a_step1"),
    (2, "paso-2", "Paso 2 — Inicio de captura", "file_match:profile_a_step2"),
    (3, "paso-3", "Paso 3 — Fin de captura", "file_match:profile_a_step3"),
    (4, "paso-4", "Paso 4 — Campos del archivo A", "file_match:profile_a_step4"),
    (5, "paso-5", "Paso 5 — Reglas de contenido", "file_match:profile_a_step5"),
    (6, "paso-6", "Paso 6 — Informe de lectura", "file_match:profile_a_step6"),
)


def get_wizard_context(project, membership=None) -> ProfileAWizardContext:
    ...
    continue_url = "file_match:profile_a_step1"
    for step in steps:
        if step.status != "done":
            continue_url = step.url_name
            break
    return ProfileAWizardContext(
...
def get_step1_catalog_context() -> dict:
    """Catálogo DMS filtrado a whitelist MVP del perfil A."""
    ...
    file_types = [
        ...
        in PROFILE_A_FILE_TYPE_WHITELIST
    ]
    ctx["file_types"] = file_types
    ctx["profile_a_whitelist"] = sorted(PROFILE_A_FILE_TYPE_WHITELIST)
    return ctx
