PROFILE_B_FILE_TYPE_WHITELIST = frozenset(
    {"csv", "xlsx", "txt_delimited", "txt_fixed", "json", "xml"}
)

WHITELIST_REJECT_MESSAGE = (
    "El tipo de archivo no está permitido en FILE MATCH (perfil B). "
    "Use CSV, Excel, TXT delimitado, TXT posicional, JSON o XML."
)


def is_allowed_profile_b_file_type(code: str | None) -> bool:
    value = (code or "").strip()
    if not value:
        return True
    return value in PROFILE_B_FILE_TYPE_WHITELIST
...
def reject_non_whitelist_file_type(code: str | None):
    ...
    value = (code or "").strip()
    if not value or value in PROFILE_B_FILE_TYPE_WHITELIST:
        return None
