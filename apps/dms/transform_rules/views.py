...
def _base_context(request, project) -> dict:
    ...
    return {
        ...
        "rules_save_url": reverse(
            "dms:transform_rules_save", kwargs={"project_slug": project.slug}
        ),
        "rules_preview_url": reverse(
            "dms:transform_rules_preview", kwargs={"project_slug": project.slug}
        ),
...
def hub(request, project_slug: str):
    ...
    return render(request, "dms/transform_rules/hub.html", _base_context(request, project))
...
def hub_help(request, project_slug: str):
    ...
    if project is None:
        return redirect("dms:mapping_list")
    return render(request, "dms/transform_rules/hub_help.html", _base_context(request, project))
...
def editor(request, project_slug: str):
    ...
    if not ctx["rule_rows"]:
        messages.warning(
            request,
            "Defina al menos un mapeo antes de configurar reglas de transformación.",
        )
        return redirect("dms:field_mapping_hub", project_slug=project_slug)
    return render(request, "dms/transform_rules/editor.html", ctx)
...
def editor_help(request, project_slug: str):
    ...
    if project is None:
        return redirect("dms:mapping_list")
    return render(request, "dms/transform_rules/editor_help.html", _base_context(request, project))
...
def rules_save(request, project_slug: str):
    ...
    if raw:
        try:
            ...
            return redirect("dms:transform_rules_editor", project_slug=project_slug)
        ...
        if isinstance(payload, dict) and "pipelines" in payload:
            ...
            pipelines = {
                (item.get("target_field") or ""): (item.get("transform_pipeline") or [])
                for item in payload
    ...
    redirect_to = request.POST.get("next", "").strip() or reverse(
        "dms:transform_rules_editor", kwargs={"project_slug": project_slug}
    )
