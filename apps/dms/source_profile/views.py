...
def _base_context(request, project, current_step: int | None = None) -> dict:
    ...
    seed_ctx = profile_seed_service.get_seed_context(
        request.user,
        project,
        destination_slot=profile_seed_service.TARGET_SLOT_SOURCE,
    )
    return {
        "project": project,
        "wizard": wizard,
...
@_source_view
def source_seed_hub(request, project_slug: str):
    project = _get_project_or_redirect(request, project_slug)
    if project is None:
        return redirect("dms:mapping_list")
...
@_source_view
def source_seed_hub_help(request, project_slug: str):
    project = _get_project_or_redirect(request, project_slug)
    if project is None:
        return redirect("dms:mapping_list")
    from_key = (request.GET.get("from") or "").strip().lower()
    if from_key == "hub":
        help_back_url_name = "dms:source_hub"
        help_back_label = "← Origen"
        help_from_hub = True
    else:
        can_import = profile_seed_service.user_can_import(request.user, project)
        if can_import:
            help_back_url_name = "dms:source_seed_hub"
...
@_source_view
def source_seed_picker(request, project_slug: str):
    project = _get_project_or_redirect(request, project_slug)
    if project is None:
        return redirect("dms:mapping_list")
    if not profile_seed_service.user_can_import(request.user, project):
...
@_source_view
def source_seed_picker_help(request, project_slug: str):
...
def source_seed_apply(request, project_slug: str):
    project = _get_project_or_redirect(request, project_slug)
    if project is None:
        return redirect("dms:mapping_list")
    if not profile_seed_service.user_can_import(request.user, project):
    ...
    if request.method == "POST":
        ...
        if action != "apply":
            ...
            return redirect("dms:source_seed_picker", project_slug=project_slug)
...
@_source_view
def source_seed_apply_help(request, project_slug: str):
    return _render(request, project_slug, "profile_seed/apply_confirm_help.html")


@_source_view
def source_seed_history(request, project_slug: str):
    project = _get_project_or_redirect(request, project_slug)
    if project is None:
        return redirect("dms:mapping_list")
...
@_source_view
def source_seed_history_detail(request, project_slug: str, event_id):
    project = _get_project_or_redirect(request, project_slug)
    if project is None:
        return redirect("dms:mapping_list")
    ...
    if detail is None:
        ...
        return redirect("dms:source_seed_history", project_slug=project_slug)
    return _render(
        request,
        project_slug,
...
@_source_view
def source_seed_history_help(request, project_slug: str):
