...
STEP4_TEMPLATES = {
    "fixed": "file_gate/schema/step4_fields.html",
    "delimited": "file_gate/schema/step4_fields_delimited.html",
...
def _base_context(request, project, current_step: int | None = None) -> dict:
    ...
    return {
        ...
        "source_save_url": reverse("file_gate:schema_save", kwargs={"project_slug": project.slug}),
...
def hub(request, project_slug: str):
    return _render(request, project_slug, "file_gate/schema/hub.html")


@_schema_view
def hub_help(request, project_slug: str):
    return _render(request, project_slug, "file_gate/schema/hub_help.html")


@_schema_view
def step1_help(request, project_slug: str):
    return _render(request, project_slug, "file_gate/schema/step1_help.html", current_step=1)


@_schema_view
def step2_help(request, project_slug: str):
    return _render(request, project_slug, "file_gate/schema/step2_help.html", current_step=2)


@_schema_view
def step3_help(request, project_slug: str):
    return _render(request, project_slug, "file_gate/schema/step3_help.html", current_step=3)
...
def step5_help(request, project_slug: str):
    return _render(request, project_slug, "file_gate/schema/step5_help.html", current_step=5)


@_schema_view
def step6_help(request, project_slug: str):
    return _render(request, project_slug, "file_gate/schema/step6_help.html", current_step=6)
...
def step1_file_type(request, project_slug: str):
    ...
    return render(request, "file_gate/schema/step1_file_type.html", ctx)
...
def step2_capture_start(request, project_slug: str):
    ...
    return render(request, "file_gate/schema/step2_capture_start.html", ctx)
...
def step3_capture_end(request, project_slug: str):
    ...
    return render(request, "file_gate/schema/step3_capture_end.html", ctx)
...
def step4_fields_delimited(request, project_slug: str):
    return redirect("file_gate:schema_step4", project_slug=project_slug)
...
def step5_content_rules(request, project_slug: str):
    ...
    return render(request, "file_gate/schema/step5_content_rules.html", ctx)
...
def step6_report(request, project_slug: str):
    ...
    # Compatibilidad con source_profile-report.js (campos ocultos, no editables aquí).
    ctx["legacy_reject_alert_threshold"] = threshold.get("value", 10)
    ctx["legacy_reject_alert_threshold_unit"] = (
        "percent" if threshold.get("mode") == "percent" else "count"
    )
    return render(request, "file_gate/schema/step6_report.html", ctx)


@_schema_view
@require_http_methods(["POST"])
def schema_save(request, project_slug: str):
    project = _get_project_or_redirect(request, project_slug)
    if project is None:
    ...
    if raw:
        try:
            ...
            if request.headers.get("X-Requested-With") == "XMLHttpRequest":
                return JsonResponse(
                    {"ok": False, "message": "JSON de contrato inválido."},
                    status=400,
                )
            messages.error(request, "JSON de contrato inválido.")
    ...
    if not redirect_to:
        redirect_to = reverse("file_gate:schema_hub", kwargs={"project_slug": project_slug})
...
@_schema_view
@require_http_methods(["POST"])
def schema_publish(request, project_slug: str):
    project = _get_project_or_redirect(request, project_slug)
    if project is None:
        if request.headers.get("X-Requested-With") == "XMLHttpRequest":
            return JsonResponse(
    ...
    result = schema_publish_service.publish_draft_schema(request.user, project)
    redirect_to = request.POST.get("next", "").strip()
    if not redirect_to:
        redirect_to = reverse("file_gate:schema_hub", kwargs={"project_slug": project_slug})
