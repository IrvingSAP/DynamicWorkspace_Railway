...
FORM_CREATE = "reverse_studio:project:create"


def _form_namespace(action: str, project_slug=None) -> str:
    if project_slug:
        return f"reverse_studio:project:{action}:{project_slug}"
    return f"reverse_studio:project:{action}"
...
def project_list(request):
    rows, stats = reverse_project_service.list_with_stats(request.user)
    ...
    return render(request, "reverse_studio/projects/list.html", ctx)


@_rs_view
def project_list_help(request):
    ctx = _sidebar_context()
    return render(request, "reverse_studio/projects/list_help.html", ctx)


@_rs_view
def project_create_help(request):
    ctx = _sidebar_context()
    return render(request, "reverse_studio/projects/create_help.html", ctx)
...
def project_create(request):
    company = request.user.profile.company
    posted = reverse_project_service.default_posted()
    ...
    if request.method == "POST":
        posted = reverse_project_service.posted_from_request(request.POST)
        result = reverse_project_service.create_project(request.user, posted)
        if result.ok:
            clear_form_state(request, FORM_CREATE)
            messages.success(request, result.user_message)
            project = result.payload["project"]
            return redirect("reverse_studio:project_hub", project_slug=project.slug)
        errors = result.errors or {}
        stash_form_state(request, FORM_CREATE, posted, errors)
        messages.error(request, result.user_message)
        return redirect("reverse_studio:project_create")
    ...
    return render(request, "reverse_studio/projects/create.html", ctx)
...
def project_hub(request, project_slug: str):
    ...
    hub = reverse_project_service.get_hub_context(request.user, project)
    ...
    return render(request, "reverse_studio/projects/hub.html", ctx)
...
def project_hub_help(request, project_slug: str):
    ...
    return render(request, "reverse_studio/projects/hub_help.html", ctx)
...
def project_members(request, project_slug: str):
    ...
    if not project_service.user_can_manage_members(request.user, project):
        ...
        return redirect("reverse_studio:project_hub", project_slug=project_slug)
    ...
    if request.method == "POST":
        ...
        if action == "invite":
            ...
            if result.ok:
                ...
                messages.error(request, result.user_message)
            return redirect("reverse_studio:project_members", project_slug=project_slug)
    ...
    return render(request, "reverse_studio/projects/members.html", ctx)
...
def project_members_help(request, project_slug: str):
    ...
    ctx = _sidebar_context()
    ctx.update({"project": project, "company": project.company})
    return render(request, "reverse_studio/projects/members_help.html", ctx)
