"""Wizard context for File Clean read profile (4 steps; no Gate content rules)."""
...
class ProfileWizardContext:
    ...
    continue_step_url_name: str = "file_clean:profile_step1"


_STEP_META = (
    (1, "paso-1", "Paso 1 — Tipo de archivo", "file_clean:profile_step1"),
    (2, "paso-2", "Paso 2 — Inicio de captura", "file_clean:profile_step2"),
    (3, "paso-3", "Paso 3 — Fin de captura", "file_clean:profile_step3"),
    (4, "paso-4", "Paso 4 — Campos del perfil", "file_clean:profile_step4"),
)
...
def get_wizard_context(project, membership=None) -> ProfileWizardContext:
    ...
    continue_url = "file_clean:profile_step1"
    for step in steps:
        if step.status != "done":
            continue_url = step.url_name
            break
    else:
        continue_url = "file_clean:profile_hub"
