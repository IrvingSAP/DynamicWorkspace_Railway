...
DEFAULT_DESCRIPTION = (
    "Proyecto File Clean: limpieza y normalización de archivos con reglas explícitas."
)

MSG_NO_ACCESS = "No tiene acceso a este proyecto File Clean."
MSG_UF_ONLY = "Solo usuarios UF pueden crear proyectos File Clean."
MSG_CREATED = "Proyecto File Clean creado correctamente."
...
def visible_projects_qs(user):
    ...
    member_ids = ProjectMembership.objects.filter(
        ...
        project__project_kind=Project.KIND_FILE_CLEAN,
...
def create_project(user, data: dict) -> OperationResult:
    ...
    try:
        with transaction.atomic():
            ...
            # Visibilidad reusa DmsProjectConfig (mismo chasis Gate/Pipe/Match).
            # CleanVersion / CleanConfig de dominio llegan con M2–M4.
        ...
        logger.exception(
            "create_file_clean_project IntegrityError slug=%s", data.get("slug")
        )
        ...
        logger.exception("create_file_clean_project unexpected")
        return OperationResult.failure(
            "unexpected",
...
def get_hub_context(user, project: Project) -> dict:
    ...
    publish = clean_publish_service.get_publish_context(project)
    ...
    rules_enabled = rules_ctx["rules_enabled_count"]
    rules_count = rules_ctx["rules_count"]
    rules_complete = rules_enabled > 0
    pending = "is-pending"
    ...
    job_label = clean_run_service.last_job_label(project)
    run_complete = CleanJob.objects.filter(
        project=project,
    ...
    if has_published:
        ...
        if run_complete:
            ...
            # Historial es consulta continua: disponible/pendiente, no «hecho».
    ...
    if rules_count:
        draft_label = f"{draft_label} · {rules_enabled}/{rules_count} reglas ON"

    modules_pending_note = ""
    if not has_published:
        modules_pending_note = (
            "Publique una versión para habilitar Ejecutar e Historial."
        )
    ...
    return {
        ...
        "continue_profile_url_name": wizard.continue_step_url_name,
