...
FORM_CREATE = "file_clean:project:create"

MSG_NO_ACCESS = clean_project_service.MSG_NO_ACCESS
MSG_PA_ONLY = clean_project_service.MSG_PA_ONLY


def _form_namespace(action: str, project_slug=None) -> str:
    if project_slug:
        return f"file_clean:project:{action}:{project_slug}"
    return f"file_clean:project:{action}"
...
def project_list(request):
    rows, stats = clean_project_service.list_with_stats(request.user)
    ...
    return render(request, "file_clean/projects/list.html", ctx)


@_fc_view
def project_list_help(request):
    ctx = _sidebar_context()
    return render(request, "file_clean/projects/list_help.html", ctx)


@_fc_view
def project_create_help(request):
    ctx = _sidebar_context()
    return render(request, "file_clean/projects/create_help.html", ctx)
...
def project_create(request):
    company = request.user.profile.company
    posted = clean_project_service.default_posted()
    ...
    if request.method == "POST":
        posted = clean_project_service.posted_from_request(request.POST)
        result = clean_project_service.create_project(request.user, posted)
        if result.ok:
            clear_form_state(request, FORM_CREATE)
            messages.success(request, result.user_message)
            project = result.payload["project"]
            return redirect("file_clean:project_hub", project_slug=project.slug)
        errors = result.errors or {}
        stash_form_state(request, FORM_CREATE, posted, errors)
        messages.error(request, result.user_message)
        return redirect("file_clean:project_create")
    ...
    return render(request, "file_clean/projects/create.html", ctx)
...
def project_hub(request, project_slug: str):
    ...
    hub = clean_project_service.get_hub_context(request.user, project)
    ...
    return render(request, "file_clean/projects/hub.html", ctx)
...
def project_hub_help(request, project_slug: str):
    ...
    return render(request, "file_clean/projects/hub_help.html", ctx)
...
def project_members(request, project_slug: str):
    ...
    if not project_service.user_can_manage_members(request.user, project):
        ...
        return redirect("file_clean:project_hub", project_slug=project_slug)
    ...
    if request.method == "POST":
        ...
        if action == "invite":
            ...
            if result.ok:
                ...
                messages.error(request, result.user_message)
            return redirect("file_clean:project_members", project_slug=project_slug)
    ...
    return render(request, "file_clean/projects/members.html", ctx)
...
def project_members_help(request, project_slug: str):
    ...
    ctx = _sidebar_context()
    ctx.update({"project": project, "company": project.company})
    return render(request, "file_clean/projects/members_help.html", ctx)
