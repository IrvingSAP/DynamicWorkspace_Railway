/**
 * Persistencia servidor — DmsTargetProfile
...
(function () {
    ...
    const dataNode = document.getElementById("dms-target-profile-data");
    const configNode = document.getElementById("dms-target-save-config");
    ...
    function merge(partial) {
        ...
        const replaceKeys = ["layout", "serialization", "write_validation"];
        const mergeKeys = ["config"];
    ...
    async function save(partial, options) {
        ...
        body.set("target_payload", JSON.stringify(payload));
        if (opts.strict) {
            body.set("strict", "1");
        }
        if (opts.next) {
